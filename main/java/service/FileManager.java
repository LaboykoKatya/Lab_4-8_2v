...
public class FileManager {
    ...
    private String serialize(Appliance a) {
        ...
        if (a instanceof KitchenAppliance) {
            KitchenAppliance ka = (KitchenAppliance) a;
            type = "Kitchen";
            extra = String.valueOf(ka.getVolume());
        } else if (a instanceof CleaningAppliance) {
            CleaningAppliance ca = (CleaningAppliance) a;
            type = "Cleaning";
            extra = String.valueOf(ca.getArea());
        } else if (a instanceof EntertainmentAppliance) {
            EntertainmentAppliance ea = (EntertainmentAppliance) a;
            type = "Entertainment";
        ...
        return String.join(";",
                type,
                escape(a.getName()),
                String.valueOf(a.getPower()),
                String.valueOf(a.isOn()),
                extra
        );
    }

    // ==========================
    //   ДЕСЕРІАЛІЗАЦІЯ (Java 8)
    // ==========================
    ...
    private Appliance deserialize(String line) {
        ...
        if ("Kitchen".equals(type)) {
            a = new KitchenAppliance(name, power, extra);
        } else if ("Cleaning".equals(type)) {
            a = new CleaningAppliance(name, power, extra);
        } else if ("Entertainment".equals(type)) {
            a = new EntertainmentAppliance(name, power, extra);
        }
